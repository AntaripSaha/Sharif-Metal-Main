                    ...
                    <div class="card-body" id="undeliveredProductTable">
                        <table id="productTable"
                            ...
                            <tbody>
                                ...
                                $in_total_amount = 0;
                                @endphp
                                @foreach ($undelivered_products as $key=>$undelivered)
                                @if($undelivered[$key][$key]->products != 0)
                                <tr class="text-center">
                                    <td>{{ $i }}</td>
                                    <td>{{ $undelivered[$key][$key]->products->product_id }}</td>
                                    <td>{{ $undelivered[$key][$key]->products->product_name }}</td>
                                    <td>{{ $undelivered[$key][$key]->undelivered_product }}</td>
                                    <td>{{ $undelivered[$key][$key]->products->price }}</td>
                                    <td>{{ $undelivered[$key][$key]->products->price * $undelivered[$key][$key]->undelivered_product }}</td>
                                </tr>
                                @php
                                $i++;
                                $in_total_amount += ($undelivered[$key][$key]->products->price * $undelivered[$key][$key]->undelivered_product)
                                @endphp
                                @endif
                                @endforeach
                            </tbody>
