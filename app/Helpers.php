<?php
    ...
    function generateRandomStr($length = 8) {
        ...
        $numbers = "0123456789";
        $characters = $numbers.$LowerStr.$UpperStr;
    ...
    if(is_array($res)){
        foreach($res as $key=>$row) {
            if((string)$selected == (string)$key)
                $code .= "<option selected='selected' value='". $key ."'>"."\t".$row."</option>";
            else
                $code .= "<option value='". $key ."'>".$row."</option>";
        }  
    }else{
        foreach($res as $row) {
            if((string)$selected == (string)$row->$value_field)
                $code .= "<option selected='selected' value='". $row->$value_field ."'>"."\t".$row->$text_field."</option>";
            else
                $code .= "<option value='". $row->$value_field ."'>".$row->$text_field."</option>";
        }
    }
    return $code;
}
